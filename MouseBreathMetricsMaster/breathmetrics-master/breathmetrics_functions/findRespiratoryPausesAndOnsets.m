...
for THIS_BREATH = 1:length(myPeaks)-1
    ...
    if ~isExhalePause
       ...
       %% Giulio Casali added this to re-align a crossing value;
       %% this does not work as good with short chunks, so commented;
       if THIS_BREATH>1 ; 
            THIS_INHALE_THRESHOLD =  nanmean(resp([inhalePauseOnsets(max(find(~isnan(inhalePauseOnsets(1:THIS_BREATH))==1))) exhaleOnsets(max(find(~isnan(inhalePauseOnsets(1:THIS_BREATH))==1)))])) ;
            if isnan(THIS_INHALE_THRESHOLD);
       ...
       if sum(POSSIBLE_INHALE_INDS == 0) == 0;            THIS_INHALE_THRESHOLD = SIMPLE_ZERO_CROSS;       POSSIBLE_INHALE_INDS = INHALE_WINDOW > THIS_INHALE_THRESHOLD; end;
       ...
       if isempty( INHALE_ONSET)
            THIS_INHALE_THRESHOLD =  nanmean(resp([inhalePauseOnsets(max(find(~isnan(inhalePauseOnsets(1:THIS_BREATH))==1))) exhaleOnsets(max(find(~isnan(inhalePauseOnsets(1:THIS_BREATH))==1)))])) ;
            POSSIBLE_INHALE_INDS = INHALE_WINDOW > THIS_INHALE_THRESHOLD;
            if sum(POSSIBLE_INHALE_INDS == 0) == 0;            
                THIS_INHALE_THRESHOLD = SIMPLE_ZERO_CROSS;       
                POSSIBLE_INHALE_INDS = INHALE_WINDOW > THIS_INHALE_THRESHOLD; 
                if sum(POSSIBLE_INHALE_INDS == 0) == 0
                    diff_INHALE = [ diff(INHALE_WINDOW), 0];
                    POSSIBLE_INHALE_INDS(find(imregionalmax(diff_INHALE),1):end) = 0;
                end
            end;
                INHALE_ONSET = find( POSSIBLE_INHALE_INDS == 0, 1, 'last' );
       end
       ...
       inhaleOnsets(1, THIS_BREATH + 1) = myTroughs( THIS_BREATH ) + INHALE_ONSET;
        ...
        %% Giulio Casali on 30/09/2020 commented the following line
                %% Giulio Casali on 16/11/2020 implemented changes to detect Linear State;
                %% On 08/03/2022 the code was reverted back to initial version to keep it consistent;
                    PlotFigure = 0 ;
                    PUTATIVE_PAUSE_INDS = [];Iteration = 0;
                    while isempty(PUTATIVE_PAUSE_INDS)
                    PUTATIVE_PAUSE_INDS = ProducePlotForCycleOfInhalationWindow(INHALE_WINDOW, [30-Iteration] ,ZERO_CROSS_THRESHOLD , PlotFigure) ;
    ...
    if ~isInhalePause
       ...
       % pause. So just use baseline crossing as inhale onset
       if THIS_BREATH>1 ;
           THIS_EXHALE_THRESHOLD =  nanmean(resp([inhalePauseOnsets(max(find(~isnan(inhalePauseOnsets(1:THIS_BREATH))==1))) exhaleOnsets(max(find(~isnan(inhalePauseOnsets(1:THIS_BREATH))==1)))])) ;
           if isnan(THIS_EXHALE_THRESHOLD);
       ...
       if sum(POSSIBLE_EXHALE_INDS)==0    
           THIS_EXHALE_THRESHOLD = EXHALE_WINDOW(knnsearch(reshape(EXHALE_WINDOW,[],1) , THIS_EXHALE_THRESHOLD ));
           POSSIBLE_EXHALE_INDS = EXHALE_WINDOW > THIS_EXHALE_THRESHOLD-1;
       end;
       ...
       exhaleOnsets(1, THIS_BREATH) = myPeaks( THIS_BREATH ) + EXHALE_ONSET;
        ...
        for ADDITIONAL_BIN = 1:5
            ...
            if nVALS_ADDED > MAX_BIN_TOTAL * BINNING_THRESHOLD
                ...
                %% Giulio Casali on 30/09/2020 commented the following line
                %% Giulio Casali on 16/11/2020 implemented changes to detect Linear State  
                %% On 08/03/2022 it reversed to make it consistent
                    %PlotFigure = 0 ;
                    %PUTATIVE_PAUSE_INDS = ProducePlotForCycleOfInhalationWindow(EXHALE_WINDOW, [30] ,ZERO_CROSS_THRESHOLD , PlotFigure) ;
                    PUTATIVE_PAUSE_INDS = find(EXHALE_WINDOW > MIN_PAUSE_RANGE  & EXHALE_WINDOW < MAX_PAUSE_RANGE);
                if ~isempty(PUTATIVE_PAUSE_INDS);
                    PAUSE_ONSET = PUTATIVE_PAUSE_INDS(1)-1;
                    EXHALE_ONSET = PUTATIVE_PAUSE_INDS(end)+1;
                else
                    PAUSE_ONSET = NaN;
                    EXHALE_ONSET =  find(POSSIBLE_EXHALE_INDS==0,1,'last') ;
                end;
        %% Now proceed with the default function
