function h = scrollplot( dx, varargin)
...
% The user can horizontally scroll the view using a scrollbar. The
% horizontal view size is dx. If yn is a row or a column the n-th subplot 
% is a line. If yn is a matrix each column is a line in the n-th subplot.
% 
% Example:
%   t = [0:1000]';
%   y1 = [sin(2*pi*t/1000) sin(2*pi*t/1000)+2];
%   y2 = 100*sin(2*pi*t/1000);
%   scrollplot(100,t,y1,t,y2);
PlotMarker = '-';
...
for sp = 1 : numsubplots
   ...
   if size(y,1) > size(y,2)
	...
	plot(x(beginind : endind), y(:,beginind : endind),PlotMarker);
	set(h(sp),'xlim',[center (center + data.dx)]);
	set(h(sp),'ylim',[min(min(y)) max(max(y))]);
end
guidata(hf, data);
